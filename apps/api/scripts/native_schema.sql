...
CREATE TABLE IF NOT EXISTS public.events (
  ...
  host_profile_id text REFERENCES public.profiles(id) ON DELETE SET NULL,
...
ALTER TABLE public.events
  ADD CONSTRAINT events_event_type_check CHECK (event_type IN ('community', 'seminar')),
  ADD CONSTRAINT events_delivery_mode_check CHECK (delivery_mode IN ('in_person', 'online', 'hybrid')),
  ADD CONSTRAINT events_topic_category_check CHECK (topic_category IN ('', 'career', 'college', 'financial_literacy', 'networking', 'other')),
  ADD CONSTRAINT events_audience_check CHECK (audience IN ('all_members', 'students', 'young_alumni', 'parents', 'college_applicants', 'career_explorers')),
  ADD CONSTRAINT events_meeting_provider_check CHECK (meeting_provider IN ('', 'zoom', 'microsoft_teams', 'google_meet', 'other')),
...
CREATE INDEX IF NOT EXISTS idx_events_tenant_type_start ON public.events (tenant_id, event_type, starts_at ASC);
CREATE INDEX IF NOT EXISTS idx_events_host_profile ON public.events (host_profile_id) WHERE host_profile_id IS NOT NULL;
...
BEGIN
  FOR tbl IN
    SELECT unnest(ARRAY[
      ...
      'newsletters', 'events', 'event_rsvps', 'event_messages', 'event_meeting_details', 'email_broadcasts', 'family_trees', 'analytics_events',
...
BEGIN
  ...
  FOR tbl IN
    SELECT unnest(ARRAY[
      ...
      'photos', 'newsletters', 'events', 'event_rsvps', 'event_messages', 'event_meeting_details', 'event_join_access_logs', 'email_broadcasts', 'family_trees',
