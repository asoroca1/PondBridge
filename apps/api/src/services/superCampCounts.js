 ...
 *
 * `profiles` is the raw row count, kept so `members + removed` can be
 * reconciled against it. It is not a number to show beside `members`: every
...
export async function loadCampCounts(tenantId, { profileModel = ProfileModel } = {}) {
  const [members, removed, profiles] = await Promise.all([
    profileModel.count({ tenantId, ...ACTIVE_ALUMNI_FILTER }),
    profileModel.count({ tenantId, status: "removed" }),
    profileModel.count({ tenantId })
  ]);

  return { members, removed, profiles };
}
