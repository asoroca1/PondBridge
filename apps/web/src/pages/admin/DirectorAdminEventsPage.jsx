...
export default function DirectorAdminEventsPage() {
  ...
  // Presenter and guest-list edits both come back with the whole detail
  // payload, so one helper keeps the pane and the agenda counts in step.
  ...
  function addPresenter(member) {
    return runRosterAction({
      path: `/events/${detailItem.id}/presenters`,
      method: "POST",
  ...
  function removePresenter(person) {
    return runRosterAction({
      path: `/events/${detailItem.id}/presenters/${person.id}`,
      method: "DELETE",
      profileId: person.id,
      label: `${person.fullName || "Member"} removed from the presenter list.`
    });
  }

  function removeAttendee(person) {
    return runRosterAction({
      path: `/events/${detailItem.id}/attendees/${person.profileId}`,
      method: "DELETE",
