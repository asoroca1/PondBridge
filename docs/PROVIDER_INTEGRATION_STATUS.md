...
| --- | --- | --- | --- |
| Cloudflare Pages/DNS | Healthy | The `pondbridge` production deployment `a64c4c10-c8c1-4cc5-8114-025fc6ed20c9` successfully published merge commit `1b358855`; the `pondbridgealumni.com` zone routes app/API/landing traffic correctly, and Cedar subdomain plus `/t/cedar` fallback smoke tests return `200`. | Review and remove obsolete explicit test/demo DNS records after owner confirmation; the wildcard already covers camp subdomains. |
...
| Supabase | Healthy but shared | Project `wkyjhmggkujsepafbplv` is active in `us-west-2`. All eight reviewed additive migrations are recorded in order; preflight reports no missing PondBridge tables/indexes, RLS/policies cover all 36 PondBridge tables, function/table privilege hardening passes, and the tenant-domain audit passes for all 11 camps. | Keep tenant cohorts empty until each feature rehearsal passes. The project also contains 21 non-PondBridge tables with permissive authenticated policies; ownership must be resolved before touching them. |
...
| OpenAI | Credential live; rollouts gated | The project-scoped key is stored in the ignored root `.env.local` and on Render's `pondbridge-api` service without being printed or committed. A direct authenticated API probe returned `200`; Render deployment `dep-d9hcs4cs728c73buj430` is live and the production health endpoint remains `200`. The GPT-5.6 Luna/Terra role split, Responses API usage, safety identifiers, token-price ledger, schemas, and offline safety evaluations pass. `feature_rollouts` and `ai_generations` remain empty, and `SUPER_COPILOT_ENABLED=false`. | Set a conservative OpenAI project spend limit, run the guarded provider evaluation against isolated synthetic staging, and then approve one synthetic or internal target camp explicitly. |
